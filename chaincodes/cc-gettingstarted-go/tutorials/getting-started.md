...
## Housekeeping
Since this tutorial will make use of ' `fabric-testnet-nano-devmode` setup you should be developing within `fabric-testnet-nano-devmode/chaincodes`. Make a folder inside `fabric-testnet-nano-devmode/chaincodes` called `cc-gettingstarted-go` and open your preferred editor there. In your terminal run the command

```
go mod init github.com/hyperledger/fabric-samples/chaincode/cc-gettingstarted-go
```
...
Begin your contract by creating a new file `simple-contract.go` within your `cc-gettingstarted-go` folder. Within this file create a struct called `SimpleContract` which embeds the `contractapi.Contract` struct. This will be our contract for managing data to and from the world state.
...
Before you must follow the steps of the [readme.md](https://github.com/kmilodenisglez/fabric-testnet-nano-devmode) to up the development network
...
> Note: ensure you have the correct permissions configured on your cc-gettingstarted-go folder for docker to create files there. Running `chmod -R 766` should set the correct permission levels.
...
```
CORE_CHAINCODE_ID_NAME=mycc:0 CORE_PEER_TLS_ENABLED=false ./cc-gettingstarted-go -peer.address peer:7052
```
